...
Thank you for your interest in contributing to *dominion-setup*!
...
To start developing *dominion-setup*, follow these instructions to setup your development environment:

1. Create a *fork* (your own personal copy) of the *dominion-setup* repository via https://github.com/sgraaf/dominion-setup/fork.
...
*dominion-setup* adheres to [the Black code style](https://black.readthedocs.io/en/stable/the_black_code_style/current_style.html) and uses [Ruff](https://docs.astral.sh/ruff/) for linting and formatting code. Make sure your code is formatted properly:
...
*dominion-setup* uses type hints/annotations throughout, and uses a combination of [mypy](https://mypy.readthedocs.io/en/stable/), [Pyrefly](https://pyrefly.org/), and [ty](https://docs.astral.sh/ty/) to do type checking. Please add type annotations for all new code, and make sure that type checking succeeds:
...
*dominion-setup* has a comprehensive test suite, which includes smoke tests, unit tests and integration tests, and uses [pytest](https://docs.pytest.org/en/stable/) to execute it:
...
*dominion-setup* uses [Google Style Python Docstrings](https://www.sphinx-doc.org/en/master/usage/extensions/example_google.html) to document its API:
...
*dominion-setup* follows the [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) format for its changelog:
...
When your pull request is approved, it will be merged into the `main` branch. Your change will only be available to users the next time *dominion-setup* is released.
